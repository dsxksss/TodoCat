...
import 'package:todo_cat/drag_and_drop_lists/drag_and_drop_list_target.dart';
import 'package:todo_cat/drag_and_drop_lists/drag_and_drop_list_wrapper.dart';
import 'package:todo_cat/drag_and_drop_lists/drag_handle.dart';
import 'package:flutter/material.dart';
import 'package:flutter/rendering.dart';
import 'package:flutter/widgets.dart';

export 'package:todo_cat/drag_and_drop_lists/drag_and_drop_builder_parameters.dart';
export 'package:todo_cat/drag_and_drop_lists/drag_and_drop_item.dart';
export 'package:todo_cat/drag_and_drop_lists/drag_and_drop_item_target.dart';
export 'package:todo_cat/drag_and_drop_lists/drag_and_drop_item_wrapper.dart';
export 'package:todo_cat/drag_and_drop_lists/drag_and_drop_list.dart';
export 'package:todo_cat/drag_and_drop_lists/drag_and_drop_list_expansion.dart';
export 'package:todo_cat/drag_and_drop_lists/drag_and_drop_list_target.dart';
export 'package:todo_cat/drag_and_drop_lists/drag_and_drop_list_wrapper.dart';
export 'package:todo_cat/drag_and_drop_lists/drag_handle.dart';
...
// 这些类型定义现在从 drag_and_drop_builder_parameters.dart 导入
// 保持向后兼容的导出
typedef ListTargetOnWillAccept = bool Function(
    DragAndDropListInterface? incoming, DragAndDropListTargetBase target);
...
typedef ItemTargetOnWillAccept = bool Function(
    DragAndDropItem? incoming, DragAndDropItemTargetBase target);
...
class DragAndDropListsState extends State<DragAndDropLists> {
  ...
  Widget _buildUnscrollableList(DragAndDropListTarget dragAndDropListTarget,
    ...
    if (widget.axis == Axis.vertical) {
      return Column(
        crossAxisAlignment: widget.verticalAlignment, // 使用 verticalAlignment
        children: _buildOuterList(dragAndDropListTarget, parameters),
      );
    } else {
      // 对于水平布局，即使 disableScrolling 为 true，也需要可滚动容器来避免溢出
      // 但使用 NeverScrollableScrollPhysics，让外层的 ScrollController 控制滚动
      return SingleChildScrollView(
        scrollDirection: Axis.horizontal,
        physics: const NeverScrollableScrollPhysics(),
        controller: _scrollController,
        child: Row(
          mainAxisAlignment: widget.horizontalAlignment, // 使用 horizontalAlignment
  ...
  Widget _buildInnerList(
    ...
    if (index == childrenCount - 1) {
      ...
      final isLastList = listIndex == widget.children.length - 1;
      
      Widget listWrapper = DragAndDropListWrapper(
        dragAndDropList: widget.children[listIndex],
        parameters: parameters,
      );
      
      // 对于水平布局，为每个列表添加右侧间距（最后一个列表除外）
      if (parameters.axis == Axis.horizontal && !isLastList) {
        listWrapper = Padding(
  ...
  _internalOnItemReorder(DragAndDropItem reordered, DragAndDropItem receiver) {
    ...
    bool itemEquals(DragAndDropItem a, DragAndDropItem b) {
    ...
    for (int i = 0; i < widget.children.length; i++) {
      if (reorderedItemIndex == -1) {
        reorderedItemIndex = widget.children[i]
                .children!
                .indexWhere((e) => itemEquals(reordered, e));
        if (reorderedItemIndex != -1) reorderedListIndex = i;
      }
      if (receiverItemIndex == -1) {
        receiverItemIndex = widget.children[i]
                .children!
                .indexWhere((e) => itemEquals(receiver, e));
    ...
    if (reorderedItemIndex == -1) {
      ...
      // 若位置未变，直接返回，避免无效回调
      if (reorderedItemIndex == receiverItemIndex &&
          reorderedListIndex == receiverListIndex) {
        return;
      }
  ...
  _internalOnListReorder(
    ...
    // 使用 key 来匹配，而不是对象相等性，因为每次重建可能创建新的对象实例
    int reorderedListIndex = widget.children.indexWhere((e) {
      if (reordered.key != null && e.key != null) {
        return reordered.key == e.key;
      }
      return reordered == e;
    });
    int receiverListIndex = widget.children.indexWhere((e) {
      if (receiver.key != null && e.key != null) {
        return receiver.key == e.key;
      }
      return receiver == e;
    });
    ...
    if (reorderedListIndex == -1) {
      ...
      // 如果位置未发生变化，则不回调，避免出现 from X to X 的情况
      if (newListIndex == reorderedListIndex) {
        return;
      }
  ...
  _internalOnItemDropOnLastTarget(DragAndDropItem newOrReordered,
      DragAndDropListInterface parentList, DragAndDropItemTargetBase receiver) {
    ...
    if (reorderedItemIndex == -1) {
      if (widget.onItemAdd != null) {
        // 新增到目标列表的最后一个位置
        widget.onItemAdd!(
            newOrReordered, receiverListIndex, receiverItemIndex);
      }
    } else {
      if (reorderedListIndex == receiverListIndex &&
          receiverItemIndex > reorderedItemIndex) {
  ...
  _internalOnListDropOnLastTarget(
      DragAndDropListInterface newOrReordered, DragAndDropListTargetBase receiver) {
    // determine if newOrReordered is new or existing
    // 使用 key 来匹配，而不是对象相等性
    int reorderedListIndex = widget.children.indexWhere((e) {
      if (newOrReordered.key != null && e.key != null) {
        return newOrReordered.key == e.key;
      }
      return newOrReordered == e;
    });
  ...
  _scrollList() async {
    if (!widget.disableScrolling &&
        ...
        mounted) {
      // 使用 addPostFrameCallback 确保在渲染完成后访问 render object
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        try {
          ...
          // 如果访问 render object 时出错，忽略错误避免崩溃
          _scrolling = false;
        }
      });
    }
