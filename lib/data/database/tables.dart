...
class Tasks extends Table {
  ...
  TextColumn get workspaceId => text()
      ...
      .withDefault(const Constant('default'))(); // 关联到 Workspace
  IntColumn get order => integer().withDefault(const Constant(0))();
  ...
  TextColumn get tagsWithColorJsonString =>
      text().withDefault(const Constant('[]'))();
  IntColumn get deletedAt => integer().withDefault(const Constant(0))();
  IntColumn get customColor => integer().nullable()(); // 自定义侧边栏颜色
  IntColumn get customIcon => integer().nullable()(); // 自定义图标 CodePoint
}

// Todos 表（关联到 Task）
class Todos extends Table {
  IntColumn get id => integer().autoIncrement()();
  TextColumn get taskUuid =>
      text().withLength(min: 1, max: 255)(); // 外键关联到 Task
  TextColumn get uuid => text().withLength(min: 1, max: 255)();
  TextColumn get title => text().withLength(min: 1)();
  IntColumn get createdAt => integer()();
  TextColumn get description => text().withDefault(const Constant(''))();
  IntColumn get priority => integer().withDefault(
      const Constant(0))(); // 0: lowLevel, 1: mediumLevel, 2: highLevel
  IntColumn get finishedAt => integer().withDefault(const Constant(0))();
...
class AppConfigs extends Table {
  ...
  TextColumn get countryCode =>
      text().withLength(min: 0, max: 10).withDefault(const Constant(''))();
  BoolColumn get emailReminderEnabled =>
  ...
  BoolColumn get backgroundAffectsNavBar =>
      boolean().withDefault(const Constant(false))();
...
class NotificationHistorys extends Table {
  ...
  IntColumn get level => integer().withDefault(
      const Constant(0))(); // 0: success, 1: info, 2: warning, 3: error
...
class CustomTemplates extends Table {
  ...
  IntColumn get createdAt => integer()();
  TextColumn get tasksJson =>
      text().withLength(min: 2)(); // JSON array of tasks
