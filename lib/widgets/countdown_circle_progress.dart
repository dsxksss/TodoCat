...
class CountdownCircleProgress extends StatefulWidget {
  ...
  @override
  State<CountdownCircleProgress> createState() =>
      _CountdownCircleProgressState();
}
...
class _CountdownCircleProgressState extends State<CountdownCircleProgress> {
  ...
  void initState() {
    ...
    _countdownTimer =
      ...
      // 计算平滑进度：基于实际经过的时间
      var newProgress = remaining / widget.totalSeconds;
      ...
      if (remainingInt > 0) {
        if (mounted) {
          setState(() {
            ...
            _remainingSeconds = remainingInt;
          });
        }
      } else {
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final textColor =
        widget.textStyle?.color ?? theme.textTheme.bodyLarge?.color;
